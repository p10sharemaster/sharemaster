...
class dboperation
{
    ...
    public function ReturnUser($user,$pass)
    {
        $array_assoc = array();

        $query = "SELECT * FROM users WHERE username='".mysqli_real_escape_string($this->mysqli,$user)."' AND password='".mysqli_real_escape_string($this->mysqli,$pass)."'";
    ...
    public function LogUserAction($userID,$datetime,$action)
    {
        $SqlQuery = "insert into user_log set user_id = ".$userID." , action_date = '".$datetime."', action = '".$action."'";
